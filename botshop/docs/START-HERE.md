...
| **Step 1 will not save** — a red banner lists fields, e.g. *“country: Use the 2-letter country code, for example KE”* | the value typed does not fit the field (spelled-out country/currency names, a broken email) | the banner lists every problem at once and marks the input in red; fix those fields and press **Save & continue** again. Blank contact fields are fine and no longer block saving |
...
| A page or action shows *“Internal server error”* with a 500 | usually a stale or hand-edited link (an id in the URL that never existed) | fixed for the panel's own links — a bad id now answers *“Not found — that link looks incomplete or out of date.”* If you still see a 500, send the `requestId` from the message and the matching `docker compose logs app` line |
